...
var $={
	...
	w:1,
	ini:function(){
		$xml.ods.fetch('content',function(v){cl(v);$.json_mod(v)},{sys:'hiro'})
		//$.db.login();
	...
	login_build:function(){
		if(_o.log_in){_o.log_in.del();}
		no.targs=_o;
		if(window.location.host=="localhost"){var loc_u="",loc_p=""}else{var loc_u="",loc_p=""};
	...
	signup_submit_2:function(o){
		...
		for (var j = 0; j < sc.length; j++) {
			if(!sc[j].getAttribute("noo") &&  sc[j].checked==0 ){
				sc[j].parentNode.style.background="#f00";
				sd=0;
			}else{
				var fdc=sc[j].getAttribute("fld");
				v[fdc]=sc[j].checked;
			}
		}
		if(sd){
	...
	menu_bot:function(){
		no.div(_o.con_bot,{_s:""},
			no.a({id:"soc_em",_s:"poi,h.40,w.40,abs,rg.20,bot.10"},function(){
				$.send_mail()
			}),
			no.a({id:"soc_fb",_s:"poi,h.40,w.40,abs,rg.70,bot.10","href":"https://www.facebook.com/heartsinrhythm","target":"_blank"},function(){
				
			}),
			no.a({id:"soc_tw",_s:"poi,h.40,w.40,abs,rg.120,bot.10","href":"https://twitter.com/HeartsInRhythm","target":"_blank"},function(){
				
			})
		);
	 },
	send_mail:function(){
	    document.location.href = "mailto:heartsinrhythm@gmail.com?subject="
	        + encodeURIComponent("HIRO Inquiry")
	...
	mid_build:function(){
		...
		var ima="i_"+Math.floor((Math.random() * 10)); 
 		no.div(_o.con_mid,{_s:"h1"},
			no.div({_s:"img/"+ima+".jpg",_s:",abs,l.0.t.0,opa.90,h1,w1,bt.fff,bw.5,background: url(img/"+ima+".jpg) no-repeat fixed center center / cover"}),
		...
		for (k in o){
			if(k=="con"){
				for(var kk in o.con){
					...
					if(ks=="http"){no.a(d,kk,{href:kk,target:"blank"});var dd=d}
					else if(ks=="tabl"){
						var db=kk.split("__");
						$.db[db[1]+"_"+db[2]](db[1],d);
					}
					else if(ks=="gmap" || ks=="vdeo"){
						var dd=no.iframe(0,d);
					}
					else if(ks=="varX"){
						var k1=kk.split("_")[1];
						var k2=k1.split(".");
